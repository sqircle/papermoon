class Numerical
  ...
  @integrate: (f, a, b, n) ->
    ...
    i = 0
    m = (n + 1) >> 1
    sum = (if n & 1 then w[i++] * f(B) else 0) -- Handle odd n
    while i < m
      Ax = A * x[i]
  ...
  ------
  findRoot: (f, df, x, a, b, n, tolerance) ->
    i = 0
    ...
    while i < n
      ...
      if fx > 0
        b = x
        x = (if nx <= a then 0.5 * (a + b) else nx)
      else
        a = x
        x = (if nx >= b then 0.5 * (a + b) else nx)
      i++
  ...
  solveQuadratic: (a, b, c, roots) ->
    ...
    if abs(a) < epsilon
      ...
      return (if abs(c) < epsilon then -1 else 0) -- Infinite or 0 solutions
    q = b * b - 4 * a * c
    return 0  if q < 0 -- 0 solutions
    q = sqrt(q)
    a *= 2 -- Prepare division by (2 * a)
    n = 0
    roots[n++] = (-b - q) / a
    roots[n++] = (-b + q) / a  if q > 0
    n -- 1 or 2 solutions
  ...
  solveCubic: (a, b, c, d, roots) ->
    ...
    return Numerical.solveQuadratic(b, c, d, roots)  if abs(a) < epsilon
    ...
    -- One real solution
    A = ((if q > 0 then -1 else 1)) * pow(abs(q) + sqrt(D), 1 / 3)
    roots[0] = A + p / A - b
    1
